...
public class PagingVO {
	...
	private int numPerPage = 3; // 하나의 페이지에 표시할 게시글의 수
	private int numPerBlock = 3; // 블록당 표시하는 페이지의 갯수
	...
	private int beginPage = 0; // 현재 블록의 시작페이지
	private int endPage = 0; // 현재 블록의 끝페이지
	...
	public void setTotalPage() {
		...
		if (totalRecord == 0 || totalRecord % numPerPage > 0) totalPage++;
	}
	
	public void setTotalBlock() {
		totalBlock = totalPage / numPerBlock;
		
		if (!(totalPage % numPerBlock == 0)) totalBlock++;
